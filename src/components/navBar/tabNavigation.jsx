...
class TabNavigation extends Component {
  ...
  getParentRouteValue = (value = this.props.location.pathname) => {
    ...
    let route = this.props.routes.filter((route) =>
      value.startsWith(route.value)
    )[1];
  ...
  generateDropdown(route, currentRoute) {
    let routes = [route, ...route.nested].map((route) => {
      const color =
        route.value.toLowerCase() === currentRoute
          ? config.primary.main
          : "black";
      return (
        <MenuItem
    ...
    return (
      <Menu
        role="complementary"
        id={`dropdown${route.value}`}
        open={Boolean(this.state[route.value])}
        value={route.value}
  ...
  render() {
    ...
    return (
      <div className="desktop">
        <Tabs
          id="navigation"
          value={this.getParentRouteValue()}
          centered
          variant="fullWidth"
